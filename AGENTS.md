...
1. `schemas/template/v2/`：字段、类型和闭集枚举。
2. `skills/ui-template/references/spec-format.md`：模板字段语义、所有权和 Authoring 行为。
3. active OpenSpec：对外可观察要求。`harden-template-lifecycle` 尚未 archive 时，effective contract 是 `openspec/specs/` base 加 `openspec/changes/harden-template-lifecycle/specs/` delta；不得为消除 pending overlay 而提前改 base。
4. `scripts/template_validation/`：上述契约的可执行实现。
5. `governance/release/`：bundle 2.0.0、兼容、迁移、回滚与分发 allowlist。
6. `README.md`、本文件和发布说明是派生入口；冲突必须修复，不能选择性忽略。

## 模板契约

schema v2 必备 `spec.md`、`tokens.yaml`、`meta.yaml`、`evidence.yaml`，可含拆分设计文档和技术栈无关的 `apply/`。`tokens.yaml` 是精确值唯一载体；origin 只允许 `source | computed | estimated | default`。模板禁止 `implementation/`、stack adapter、工程目录、依赖、API/mock/data 分层、状态库选型和 runnable starter。

`templates/workbench-shell/` 的来源是两项并列来源：固定 revision `879d0de9166261c26ec35b69f5cec9382191eda1` 的公开 `multica-ai/multica` 仓库源码，以及 revision `0aedb680ecdf61aa8eafdb5d80e6b58edba63df5` 的用户 Markdown 布局设计文档（出处已抹除、业务实体已泛化）。准确身份见 `templates/workbench-shell/meta.yaml`，token/default provenance 见 `evidence.yaml`；不得从被排除样例反推模板决定。
