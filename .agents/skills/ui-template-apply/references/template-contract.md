...
5. 可选 `fidelity.yaml`：受支持 `repo-structural-v1` 进入 included/deferred/excluded；合法 v2 无 sidecar 明确记录 `legacy-baseline`；用户明确 style-only receipt 与 legacy 不同；未知 schema/profile 停止并要求兼容升级，不得忽略 sidecar。无 chrome-complete sidecar 时不得把 layout 当 high / profile-verified。
