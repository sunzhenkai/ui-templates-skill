...
description: 本仓库 templates/ 的项目级薄路由。创建/导入/更新 schema v2 模板时转到 skills/ui-template-author；使用已有模板实现页面时转到 skills/ui-template-apply；建立或重构设计系统时转到 skills/ui-template-design。仅用于 ui-templates-skill 仓库，不属于公开 bundle。
metadata:
  internal: true
---
...
本文件只负责路由和仓库位置约定，不拥有 Authoring/Apply/Design 流程或格式定义。
...
- “建立 / 重构设计系统 / 统一 token 与 Pattern” → 读取并执行 `skills/ui-template-design/SKILL.md`。
...
- `skills/` 是生产正文唯一源码。本 manager 仅为 repository-only wrapper，不进入公开 bundle。
- 改 Authoring/格式只改 `skills/ui-template-author/`；改 Apply 只改 `skills/ui-template-apply/`；改 Design 只改 `skills/ui-template-design/`。不得在本文件复制通用阶段、schema 字段或 checker 实现。
