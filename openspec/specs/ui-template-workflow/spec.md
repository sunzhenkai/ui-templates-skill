...
定义 `ui-template` skill 的模板创建/导入与模板消费落地双工作流，使 Agent 在用模板实现 UI 时必须按阶段推进，并以设计系统、可访问性、路由状态和真实浏览器验收作为完成门禁。

## Requirements

### Requirement: 双工作流入口
`ui-template` skill SHALL 明确区分 Template Authoring 与 Template Apply 两个工作流。Template Authoring SHALL 用于从 web、repo 或 image 提取、导入和维护模板；Template Apply SHALL 只能在用户选择已有模板后启动。
...
- **WHEN** 用户给定 URL、仓库或图片并要求沉淀风格
- **THEN** skill 进入 Template Authoring 工作流，并按来源生成或更新模板

#### Scenario: 尚无合适模板
- **WHEN** Template Apply 阶段发现没有可满足需求的模板
- **THEN** skill 先引导用户创建或扩展现有模板，再继续消费端流程

### Requirement: 阶段化 Apply 工作流
Template Apply SHALL 定义且不得跳过以下阶段：模板选择与范围确认、美学方向与设计系统、IA/layout/route、代码目录结构、基础组件、页面与垂直切片、完整页面模式实现、浏览器验证、design review、模板反馈评估。每个阶段 SHALL 说明输入、产物、工具配合和验收门禁。
...
### Requirement: 美学方向先行
Template Apply SHALL 在生成 UI 代码前产出明确的美学承诺，包括风格关键词、明暗主题、配色角色、字体阶梯、密度、圆角、边框与阴影策略。后续实现 SHALL 将颜色、字号、间距、圆角和层级映射到语义 token 或已确认的模板规则，而不是散落任意值。
...
Template Apply SHALL 在编写 composed business component 前产出 route inventory、App Shell 形态、页面模式、断点行为、导航语义和 URL 状态约定。路由入口 SHALL 使用真实 link 语义表达，当前路由 SHALL 使用当前页语义标记。
...
Template Apply SHALL 在组件组装前确认代码目录结构、命名规则、状态管理、数据访问、路由实现和测试组织。后续新增文件 SHALL 有唯一且明确的归属。
...
Template Apply SHALL 在页面组装前维护组件 inventory。每个组件条目 SHALL 说明用途、语义元素、variants、尺寸、交互状态、可访问性要求和来源；使用 React/Tailwind 项目时 SHOULD 优先检查 shadcn 是否已有生产组件。
...
Template Apply SHALL 将以下工具作为默认 adapter：`ui-ux-pro-max` 用于风格、配色、字体和 UX 规则检索；`frontend-design` 用于美学承诺；shadcn 用于组件检索与生产组件来源；Playwright MCP、chrome-devtools MCP 或 browser-use 用于真实浏览器反馈；design review 或等效 checklist 用于完成前审查。某个工具不可用时 SHALL 记录回退方案，流程仍须达到同等验收目标。
...
### Requirement: 真实浏览器验证
Template Apply SHALL 在收尾前用真实浏览器验证渲染结果。验证 SHALL 覆盖桌面与窄屏视口、控制台错误、可访问性树、computed style、关键交互状态、加载/空/错误状态和路由恢复。验证产物 SHALL 保留截图或可复查的检查结果。
...
Template Apply SHALL 以多阶段 design review 作为完成门禁。review SHALL 覆盖视觉一致性、响应式、交互状态、可访问性、路由语义、模板 token 符合度和关键用户流程。
...
### Requirement: 模板反馈闭环
Template Apply 完成前 SHALL 评估实现过程中发现的规则缺口。若问题会重复出现在其他消费者中，Agent SHALL 将其沉淀回模板或通用 skill；仅属于当前业务的问题 SHALL 记录在项目内，不得污染模板。

#### Scenario: 发现可复用规则缺口
- **WHEN** 实现发现模板未定义 icon-only 控件、路由状态或错误态的可复用规则
- **THEN** Agent 更新对应模板文档或通用 skill 引用文档

#### Scenario: 仅当前业务需要特殊样式
- **WHEN** 问题只来自某个业务页面的产品要求
- **THEN** Agent 将决定记录在项目实现说明中，而不是修改通用模板
