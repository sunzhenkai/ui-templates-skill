...
- 扩展 validator 与 Authoring source replay：校验 profile schema、token/rule/source 引用、scroll owner、上下文冲突、盒模型完整性、禁入工程内容及 unresolved 状态。**Session source**（本次 Generate 用户明确给出的可读来源）才绑定 source-ID→root 并重放 locator/span；**provenance**（`meta.sources[]`）只是已发布模板的出处身份，不是文件系统路径。portable validation 不假设来源仓库仍存在，也不得因 provenance 向用户索要历史 checkout。
...
- 将 `templates/workbench-shell/` 作为已发布 repo+doc 模板的边界实例：无本会话 source 时保持 core v2 `legacy-baseline`，不索取本地上游路径、不把 snapshot locator 冒充 source-direct。Board/Dialog/link/overlay 的 structural 正向与负向证据由非 example fixtures 承担；只有用户后续明确提供与声明 revision 一致的 session source 时才允许补 source-direct sidecar。
