...
### 6. Validator 分 portable 与 source-bound 两层

Portable validation 始终运行：profile JSON Schema、重复键/ID、closed enums、token/rule/source refs、scroll domain、region/overlay refs、geometry 完整性、context conflicts、unresolved 状态、canonical digest、prohibited engineering content 和机器计数。

Source replay 仅在 Authoring 有授权 checkout 时运行。CLI/runtime 使用显式 source-ID→root 绑定，验证 root 边界、revision、locator、span digest、capture receipt 和 published record；不从 PATH、网络或其他 checkout 猜测。JSON 报告分别给出 `declared/resolved/executed/passed`。Portable 环境无 source root 时报告 `not-run`；structural Authoring completion 要求 required replay 全部 executed/passed。

所有 source paths 做 realpath/symlink/parent traversal 防护。Replay 只读，不执行来源命令，也不把源码内容写入 bundle/report。
...
### 8. Workbench-shell 作为模板 pilot，但不使用 example 实现

Pilot 只读取 `meta.yaml` 已声明的固定上游 repo revision、已泛化 doc revision和模板当前 active docs/tokens；首先重建 direct source provenance，旧模板 snapshot locator 只能作为 migration history，不能证明上游 observation。

Profile 最小 included scope：Web shell/canvas、Board、master/detail、modal registry、Dialog content/header/close/footer、navigation/entity-row/button/prose link contexts。`apply/quality.md` 只引用 profile record IDs 与取证方式。
...
- **[Risk] Workbench pilot 依赖外部上游 checkout。** → 固定 revision、显式本地 source root、无网络 fail closed；fixtures 保证离线 CI，pilot replay 作为单独可审计 gate。
...
6. **Workbench pilot**：仅从声明的 repo/doc revisions 生成 `templates/workbench-shell/fidelity.yaml`，修正 direct provenance，更新模板 docs/quality/INDEX 必要一致性；不访问 `example/**`。
