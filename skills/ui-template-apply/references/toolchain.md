...
binding 声明的 component system 在已加载 vendor 参考中提供壳层原语时（如 `component_system: shadcn` 的 SidebarProvider/Sidebar/SidebarInset），壳层 SHALL 从该原语起步消费其 inset/variant 语义，SHALL NOT 手写等价结构绕过；偏离必须记录理由并获用户确认后写入 checkpoint。
