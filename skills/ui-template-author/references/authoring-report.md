...
`style-only` 必须报告非空选择理由、空 structural facets、replay `not-run` 和降级文本“未提供 structural layout/geometry/state fidelity”；chrome composition 亦未提供；不得写成 profile-verified。合法 v2 无 sidecar 的消费/迁移/已发布模板 portable 报告使用 `legacy-baseline` 和“structural profile unavailable”，replay 可为 `not-run`，不能冒充用户主动 style-only，也不能把缺历史 checkout 写成失败。未知 sidecar/profile 不属于 baseline，必须 fail closed。页面模式分类学覆盖来源壳 IA、缺 graph 或 chrome incomplete 时不得 Index。
