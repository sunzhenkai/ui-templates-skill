...
本文定义 workbench-shell 落地时必须覆盖的核心组件。每个实现条目都应能回答:语义元素是什么、有哪些 variants、尺寸、状态、键盘与辅助技术行为、来源是什么。

视觉值以 [`../spec.md`](../spec.md) 为准;本文不重复色值和密度表。
...
- `shadcn`:使用 shadcn 生产组件,按模板 token 适配。
- `compose`:用 shadcn/headless primitives 组合。
- `custom`:没有等价组件或行为差异显著;必须说明理由。
- `existing`:目标项目已有组件;必须验证状态与可访问性。
...
source:          shadcn Dialog 或 headless drawer
...
source:          shadcn Breadcrumb 或 compose
usage:           模式 C 与深层实体
```
...
source:          custom 或 shadcn Textarea
usage:           事件描述、评论、通知模板
```
...
source:            shadcn Combobox / compose
...
source:          custom 或 shadcn Pagination
usage:           事件列表、表格
```
