# workbench-shell · Implementation Playbook

本文是 `workbench-shell` 的消费端实施总入口。它不复制设计规则;所有布局、密度、颜色、chrome、断点和交互约束以 [`../spec.md`](../spec.md) 为准。平台外壳差异见 [`../platforms/web.md`](../platforms/web.md)、[`../platforms/mobile.md`](../platforms/mobile.md) 和 [`../platforms/desktop.md`](../platforms/desktop.md)。
...
- 本模板不提供 runnable starter;消费者应在自己的项目中建立实现。
- React + Vite + Tailwind + shadcn 是默认 adapter,不是唯一技术栈。

## 必读文件

| 文件 | 用途 |
| --- | --- |
| [`../spec.md`](../spec.md) | 设计规则唯一入口 |
...
| [`components.md`](components.md) | 核心组件 inventory、状态和可访问性 |
| [`code-structure.md`](code-structure.md) | 目录、命名、状态和数据边界 |
| [`stack-react-vite-tailwind-shadcn.md`](stack-react-vite-tailwind-shadcn.md) | 默认技术栈 adapter |
...
1. 按 [`code-structure.md`](code-structure.md) 确认目录。
...
| 实施顺序、route 模板、组件清单缺口 | 本目录对应文档 |
| React/Tailwind/shadcn 专属问题 | [`stack-react-vite-tailwind-shadcn.md`](stack-react-vite-tailwind-shadcn.md) |
