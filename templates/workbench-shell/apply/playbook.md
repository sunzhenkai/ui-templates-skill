...
| --- | --- | --- | --- | --- |
| Phase 0 — Intake | schema、coverage、目标平台、A–E route scope、legacy 文档详情、legacy-baseline intake | @TOKEN-001、@ROUTE-005、@ROUTE-006、@ROUTE-007、@ROUTE-008、@ROUTE-009、@RESP-001 | validation 结果与 included/deferred/excluded decision | schema/evidence/rule refs 可解析；intake 记录 `legacy-baseline` 与 `structural fidelity unavailable`（当前无 `fidelity.yaml`），缺 sidecar 不是索要来源路径的理由；`meta.sources[]` 只是出处身份，不得作为实现输入、打开对应 checkout 或按 ref clone；每个 route 映射 A–E；legacy 文档详情映射 B 或 excluded；Web/Mobile/Desktop 路径不混同。 |
